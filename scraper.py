...
Agregar más productos en la lista PRODUCTS.
"""
...
def generar_nombre_colombiano(seed: int = None) -> str:
    """Genera un nombre colombiano aleatorio."""
    if seed is not None:
        random.seed(seed)
    return random.choice(NOMBRES_COLOMBIA)
...
def generar_email(nombre: str, seed: int = None) -> str:
    ...
    # Estrategias de generación de email
    estrategias = []
...
# ─── Agregar aquí los productos a scrapear ────────────────────────────────────
PRODUCTS = [
    {
        "url": "https://www.mercadolibre.com.co/vaseline-cocoa-radiant-body-oil-humectante-corporal-200ml/p/MCO23764391?pdp_filters=item_id%3AMCO2316153936",
        "product_handle": "vaseline-cocoa-radiant-body-oil-200ml",
        "product_id": "",
    },
    # Copiar y pegar para más productos:
    # {
    #     "url": "https://www.mercadolibre.com.co/...",
    #     "product_handle": "handle-en-shopify",
    #     "product_id": "",
    # },
]
...
def generate_fake_email(name: str, index: int) -> str:
    clean = re.sub(r'[^a-z0-9]', '', name.lower()) or "cliente"
    return f"{clean}{index}@review.ml"


def scrape_product(page, product: dict) -> list[dict]:
    url = product["url"]
    print(f"\n{'='*60}")
    ...
    print("[1/3] Cargando página del producto...")
    ...
    print("[2/3] Buscando reviews...")
    page.evaluate("window.scrollTo(0, document.body.scrollHeight * 0.3)")
    time.sleep(2)

    # Intentar obtener iframe
    ...
    if not iframe_src:
        # Fallback: construir URL desde el product ID
    ...
    all_reviews = []
    offset = 0
    limit = 15  # Máximo permitido por la API de ML
    max_pages = 30
    ...
    # 4. Extraer imágenes del DOM (la API no las incluye)
    print("[4/4] Extrayendo imágenes de las reviews...")
    page.goto(iframe_url, wait_until="domcontentloaded", timeout=30000)
    time.sleep(3)

    # Scroll progresivo para cargar todas las reviews en el DOM
    # y así extraer las imágenes de todas
    image_map = {}
    prev_count = 0
    for scroll_round in range(15):  # ~15 rounds to load all 187 reviews
        ...
        if current_count == prev_count:
            # No new reviews loaded, try scrolling up and down
    ...
    # Now extract all images from the loaded DOM
...
def format_for_judgeme(api_reviews: list, image_map: dict, product: dict) -> list[dict]:
    ...
    # Mezclar nombres para asignación aleatoria pero determinista
    random.seed(42)
    nombres_asignados = random.sample(NOMBRES_COLOMBIA, min(len(NOMBRES_COLOMBIA), max(len(api_reviews), 50)))
    # Si hay más reviews que nombres, repetir
    ...
    for i, r in enumerate(api_reviews):
        ...
        # Nombre colombiano aleatorio (no "Cliente verificado")
        ...
        # Match images by review body text
        picture_urls = ""
        if body:
            key = body[:60]
            if key in image_map:
...
def main():
    ...
    if all_reviews:
        # utf-8-sig agrega BOM para que Excel abra correctamente acentos y ñ
